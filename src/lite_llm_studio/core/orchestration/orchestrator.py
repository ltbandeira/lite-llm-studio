...
class Orchestrator:
    ...
    def run_full_pipeline(self) -> bool:
        ...
        try:
            ...
            self.logger.debug("Pipeline Stage 1/1: Hardware Detection")

            # Execute hardware scan
            self.logger.info("Executing hardware scan...")
            hardware_info = self.execute_hardware_scan()

            if hardware_info is None:
                self.logger.error("Hardware scan failed, aborting pipeline")
                self.logger.warning("Pipeline execution terminated prematurely")
                return False

            self.logger.info("Hardware scan stage completed successfully")
            ...
            if pipeline_start_time:
                execution_time = time.time() - pipeline_start_time
                self.logger.error(f"Failed after {execution_time:.2f} seconds")
